...
public class ParticleOutcome extends Outcome {
    ...
    public ParticleOutcome(int luck, float chance, Optional<Integer> delay, Optional<PosProvider> pos, boolean reinit, ParticleEffect particle, IntProvider count, Optional<PosProvider> velocity, Optional<Double> speed) {
    ...
    public void run(Context context) {
        ...
        for (ServerPlayerEntity serverPlayer : context.world().getServer().getPlayerManager().getPlayerList()) {
            for (int i = 0; i < count.get(context.world().getRandom()); ++i) {
                ...
                if (shouldReinit()) {
                    pos = getPosProvider().isPresent() ? getPosProvider().get().getVec(context) : context.pos().toCenterPos();
                    if (this.velocity.isPresent()) {
                        velocity = this.velocity.get().getVec(context);
                    }
                }
