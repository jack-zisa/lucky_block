...
public class FeatureOutcome extends Outcome {
    ...
    public void run(OutcomeContext context) {
        if (context.world() instanceof ServerWorld serverWorld && serverWorld.getServer().getRegistryManager() instanceof DynamicRegistryManager dynamicRegistryManager) {
            Identifier id = featureId.get(serverWorld.getRandom().nextInt(featureId.size()));
            ConfiguredFeature<?, ?> configuredFeature = dynamicRegistryManager.get(RegistryKeys.CONFIGURED_FEATURE).get(id);
            ...
            if (!placementModifiers.isEmpty()) {
                PlacedFeature placedFeature = new PlacedFeature(dynamicRegistryManager.get(RegistryKeys.CONFIGURED_FEATURE).getEntry(id).get(), placementModifiers);
