        ...
        try {
            for (Path addonPath : Files.walk(addonsPath, 1).toList()) {
                ...
                if (Files.isDirectory(addonPath)) {
                    Path assetPath = addonPath.resolve(resourceType == ResourceType.SERVER_DATA ? "data" : "assets").resolve(id.getNamespace()).resolve(id.getPath());
                    ...
                    try (FileSystem fileSystem = FileSystems.newFileSystem(addonPath, (ClassLoader) null)) {
                        Path assetPath = fileSystem.getPath(resourceType == ResourceType.SERVER_DATA ? "data" : "assets").resolve(id.getNamespace()).resolve(id.getPath());
                        if (Files.exists(assetPath)) {
    ...
    public void findResources(ResourceType type, String namespace, String prefix, ResultConsumer consumer) {
        ...
        try {
            Files.walk(addonsPath, 1).forEach(addonPath -> {
                if (!addonPath.equals(addonsPath)) {
                    if (Files.isDirectory(addonPath)) {
                        Path namespacePath = addonPath.resolve(resourceType == ResourceType.SERVER_DATA ? "data" : "assets").resolve(namespace);
