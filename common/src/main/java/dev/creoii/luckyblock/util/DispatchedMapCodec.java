...
public record DispatchedMapCodec<K, V>(Codec<K> keyCodec, Function<K, Codec<? extends V>> valueCodecFunction) implements Codec<Map<K, V>> {
    public static <K, V> Codec<Map<K, V>> of(final Codec<K> keyCodec, final Function<K, Codec<? extends V>> valueCodecFunction) {
    ...
    private <T> DataResult<Unit> parseEntry(final DataResult<Unit> result, final DynamicOps<T> ops, final Pair<T, T> input, final Map<K, V> entries, final Stream.Builder<Pair<T, T>> failed) {
        ...
        final Optional<Pair<K, V>> entry = entryResult.result();
        if (entry.isPresent()) {
